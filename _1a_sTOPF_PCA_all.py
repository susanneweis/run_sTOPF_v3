...
def main(base_path, proj, code, movies_properties): 
    ...
    results_path = f"{base_path}/results_run_sTOPF_{code}_data_{proj}/results_PCA_all"
    ...
    # Define the output directory
        ...
        #else:
        output_dir = f"{results_path}/{curr_mov}" # Local results directory
        ...
        if not os.path.exists(output_dir):
            ...
            pc1_loadings_all_allROIs = []
            pc2_loadings_all_allROIs = []
            pc1_scores_all_allROIs = []
            pc2_scores_all_allROIs = []
            explained_variance_1_all_allROIs = []
            explained_variance_2_all_allROIs = []
            ...
            for region in brain_regions:        
                ...
                pc_loadings_all, pc_scores_all, explained_variance_all_1, explained_variance_all_2  = perform_pca(standardized_matrix)
                if pc_loadings_all is not None:
                    for idx, row in pc_loadings_all.iterrows():
                        pc1_loadings_all_allROIs.append([region, row['Subject_ID'], row['PC1_loading']])
                        pc2_loadings_all_allROIs.append([region, row['Subject_ID'], row['PC2_loading']])
                    for idx, value in enumerate(pc_scores_all['PC1_score']):
                        pc1_scores_all_allROIs.append([region, value])
                    for idx, value in enumerate(pc_scores_all['PC2_score']):
                        pc2_scores_all_allROIs.append([region, value])                   
                    explained_variance_1_all_allROIs.append([region, explained_variance_all_1])
                    explained_variance_2_all_allROIs.append([region, explained_variance_all_2])
                
            # Save the PCA results to CSV files for each gender
            pd.DataFrame(pc1_loadings_all_allROIs, columns=["Region", "Subject_ID", "PC_loading_1"]).to_csv(f"{output_dir}/PC1_loadings_all_allROI.csv", index=False)
            pd.DataFrame(pc2_loadings_all_allROIs, columns=["Region", "Subject_ID", "PC_loading_2"]).to_csv(f"{output_dir}/PC2_loadings_all_allROI.csv", index=False)
            pd.DataFrame(pc1_scores_all_allROIs, columns=["Region", "PC_score_1"]).to_csv(f"{output_dir}/PC1_scores_all_allROI.csv", index=False)
            pd.DataFrame(pc2_scores_all_allROIs, columns=["Region", "PC_score_2"]).to_csv(f"{output_dir}/PC2_scores_all_allROI.csv", index=False)
            pd.DataFrame(explained_variance_1_all_allROIs, columns=["Region", "explained_variance_1"]).to_csv(f"{output_dir}/explained_variance_1_all_allROI.csv", index=False)
            pd.DataFrame(explained_variance_2_all_allROIs, columns=["Region", "explained_variance_2"]).to_csv(f"{output_dir}/explained_variance_2_all_allROI.csv", index=False)
    ...
    output_dir = f"{results_path}/concat" # Local results directory
    if not os.path.exists(output_dir):
        ...
        for region in brain_regions:        
            ...
            #combined_matrix = combined_matrix.loc[:, :]
            ...
            # Perform PCA for females
            pc_loadings_all, pc_scores_all, explained_variance_all_1, explained_variance_all_2 = perform_pca(combined_matrix)
            if pc_loadings_all is not None:
                for idx, row in pc_loadings_all.iterrows():
                    pc1_loadings_all_allROIs.append([region, row['Subject_ID'], row['PC1_loading']])
                    pc2_loadings_all_allROIs.append([region, row['Subject_ID'], row['PC2_loading']])
